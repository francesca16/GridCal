...
def eval_f(x: Vec, Cg, k_m: Vec, k_tau: Vec, ig: Vec, Sbase: float, Pref: Vec, slack_bus_num: Vec) -> Vec:
    """
    ...
    # Pg[slack_bus_num]=0
    # Pref[slack_bus_num]=0
    Pdiff=Pg-Pref
    # Pdiff[slack_bus_num]=0
    fval = np.sum(np.power(Pdiff,2)) * 1e-4
...
def jacobians_and_hessians(x, Cg, Cf, Ct, Yf, Yt, Ybus, Sbase, il, ig, nig, slack, no_slack, pq, pv, tanmax,
    ...
    if compute_jac:
        ...
        #Pg_copy[slack_bus_num]=0

        fx[2 * N: 2 * N + Ng] = (-2*Pref + 2*Pg_copy)*Sbase * 1e-4
    ...
    if compute_hess:
        ...
        fxx_pg=2*np.ones(Ng)
        #fxx_pg[slack_bus_num]=0
        fxx = diags((np.r_[np.zeros(2 * N), fxx_pg , np.zeros(Ng)]) * 1e-4).tocsc()
