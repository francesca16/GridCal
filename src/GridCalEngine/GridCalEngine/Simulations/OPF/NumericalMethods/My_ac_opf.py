...
from GridCalEngine.Simulations.OPF.NumericalMethods.My_ac_opf_derivatives import (x2var, var2x, eval_f, eval_g, eval_h,
...
def compute_analytic_structures(
                                x, mu, lmbda, compute_jac: bool, compute_hess: bool, Ybus, Yf, Cg, Cf, Ct, R, X,
                                ...
                                k_tau, k_mtau, c0, c1, c2, Sbase, rates, il, ig, nig, Sg_undis,
                                Pref,slack_bus_num,
                                ) -> IpsFunctionReturn:
    """
    ...
    f = eval_f(x=x, Cg=Cg,k_m=k_m, k_tau=k_tau, ig=ig, Sbase=Sbase, Pref=Pref, slack_bus_num=slack_bus_num)
    ...
    fx, Gx, Hx, fxx, Gxx, Hxx = jacobians_and_hessians(x=x, Cg=Cg, Cf=Cf, Ct=Ct, Yf=Yf, Yt=Yt, Ybus=Ybus,
                                                       ...
                                                       Pref=Pref, slack_bus_num=slack_bus_num)
...
def evaluate_power_flow_debug(x, mu, lmbda, compute_jac: bool, compute_hess: bool,
    ...
    mats_analytic = compute_analytic_structures(Pref, slack_bus_num,
                                                x, mu, lmbda, compute_jac, compute_hess,
...
def ac_optimal_power_flow(Pref,
                          slack_bus_num,
                          nc: NumericalCircuit,
        ...
        if use_autodiff:
            ...
            result = interior_point_solver(x0=x0, n_x=NV, n_eq=NE, n_ineq=NI,
                                           ...
                                           arg=(Ybus, Yf, Cg, Cf, Ct, R, X, Sd, slack, no_slack, Yt, from_idx, to_idx,
                                                ...
                                                k_m, k_tau, k_mtau, c0, c1, c2, Sbase, rates, il, ig, nig, Sg_undis, Pref, slack_bus_num),
                                           verbose=pf_options.verbose,
                                           max_iter=pf_options.max_iter,
                                           tol=pf_options.tolerance,
